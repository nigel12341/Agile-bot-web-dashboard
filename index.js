...
/* Client Variables */
const client_id = '1066056964083298415'; // Paste your bot's ID here
const client_secret = 'wuR-FGikqNn2gNH0ZYzfsfs9u4cDx5d_'; // Paste your bot's secret here

/* Define app variables */
const app = Express(); // Create a web app
const port = 5000; // Port to host on
...
app.post('/user', (req, res) => { // Will run when there are any incoming POST requests to http://localhost:(port)/user. Note that a POST request is different from a GET request, so this won't exactly work when you actually visit http://localhost:(port)/user
    ...
    data_1.append('redirect_uri', `http://localhost:8080`); // This is the redirect URL where the user will be redirected when they finish the Discord login
    ...
    fetch('https://discord.com/api/oauth2/token', { method: "POST", body: data_1, headers: {'Content-Type': 'application/x-www-form-urlencoded'
    }}).then(response => response.json()).then(data => { // Make a request to the Discord API with the form data, convert the response to JSON, then take it and run the following code.
