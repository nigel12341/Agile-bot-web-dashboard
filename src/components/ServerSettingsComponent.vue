  ...
  <div id="settings" v-if=setupStatusBackend>
    <div id="botSettings" class="card">
      ...
      <div id="changeSettings">
        ...
        <h5>Please note the id you input will not be checked if they are correct. For a better setup run /setup in discord!</h5>
        ...
        <div id="ticketCatagory">
          ...
          <button id="ticketCatagoryButton" class="btn btn-primary" @click="changeTicketCatagory">Change ticket catagory</button>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    async fetchSettings() {
      await onSnapshot(doc(db, "Guilds", this.$route.query.id), (doc) => {
        ...
        this.setupStatus = ((doc.data().setup) ? "Setup complete" : "Setup incomplete");
